...
app.get('/rooms/:roomCode', (_req, res) => {
  ...
  if (!username || username == "") res.redirect(`/username?roomcode=${roomCode}`)
  else res.sendFile(path.join(STATIC_DIR, '/html/room.html'));
});

app.get('/username', (_req, res) => {
  const {roomcode} = _req.query;
  if (!roomcode || roomcode === "") res.redirect('/');
  else res.sendFile(path.join(STATIC_DIR, '/html/username.html'));
})
...
app.post('/rooms', (req, res) => {
  ...
  if (!username || username == "") res.redirect(`/username?roomcode=${roomCode}`)
  else res.redirect(`/rooms/${roomCode}?username=${username}`);
});
