...
export function createHUD(): void {
  ...
  HUD.kills.graphic = room.game.game.add.sprite(
      10, HUD.ammo.graphic.y + HUD.ammo.graphic.height + 10, 'crosshairs');
  ...
  HUD.kills.text = room.game.game.add.text(
      HUD.kills.graphic.x + 4 + HUD.kills.graphic.width,
  ...
  HUD.deaths.graphic = room.game.game.add.sprite(
      HUD.kills.text.x + HUD.kills.text.width + 5, HUD.kills.graphic.y - 5,
      'survivor_1');
  ...
  HUD.deaths.text = room.game.game.add.text(
      HUD.deaths.graphic.x - 3 + HUD.deaths.graphic.width, HUD.deaths.graphic.y,
      '0', {
        font: 'bold 30px Annie Use Your Telescope',
        fill: '#ffffff',
        align: 'center'
  ...
  HUD.radar.overlay.drawRect(
      0, 0, GAME_BOARD_WIDTH / 20 + 8, GAME_BOARD_HEIGHT / 20 + 8);
  // HUD.radar.endFill();
